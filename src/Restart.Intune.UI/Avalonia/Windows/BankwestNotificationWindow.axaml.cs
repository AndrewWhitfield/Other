namespace Restart.Intune.UI.Avalonia.Windows
{
    ...
    // MAPPING NOTES
    // ─────────────────────────────────────────────────────────────────────────
    // [B4]  RestartWindowBase (WPF) cannot be reused. Identical event handler
    //       logic reproduced here; business logic delegated to the same static
    //       methods on RestartWindowBase — no rule duplication.
    //
    // [B8]  WPF Window.Loaded → Avalonia Window.Opened.
    //
    // The only difference from CBANotificationWindow.axaml.cs is the brand
    // string passed to HyperlinkNavigationHandler ("BankwestNotificationWindow").
    // ─────────────────────────────────────────────────────────────────────────

    public partial class BankwestNotificationWindow : Window
    {
        ...
        public BankwestNotificationWindow()
        {
        ...
        private void HelpLink_Click(object sender, RoutedEventArgs e)
        {
            if (!string.IsNullOrEmpty(ViewModel.HelpUrl))
                HyperlinkNavigationHandler.Handle(ViewModel.HelpUrl, _browserLauncher, "BankwestNotificationWindow");
        }
